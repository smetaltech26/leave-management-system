    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-1">รายชื่อพนักงาน ผู้อนุมัติ และการบันทึก LINE User ID สำหรับ Direct Push Notification</p>
        </div>
      </div>

      {/* Info Card */}
      <div className="glass-card rounded-2xl p-4 border border-emerald-500/30 bg-emerald-500/5 text-xs text-slate-300 flex items-start space-x-3">
      ...
      {/* Users Table */}
      <div className="glass-card rounded-3xl p-4 md:p-6 border border-slate-800 shadow-2xl overflow-x-auto">
            ...
            {users.map((u) => {
              ...
              return (
                <tr key={u.id} className="hover:bg-slate-900/40 transition-all">
                  ...
                  <td className="py-3.5 px-3 text-slate-400">{u.agency_id || 'SMT'} / {u.department_id || 'ทั่วไป'}</td>
                  <td className="py-3.5 px-3">
                  ...
                  <td className="py-3.5 px-3 text-slate-300">{app1 ? app1.fullname : '-'}</td>
                  <td className="py-3.5 px-3">
                    {u.line_user_id ? (
                  ...
                  <td className="py-3.5 px-3 text-right">
                    <button
                      ...
                      className="py-1.5 px-3 bg-slate-800 hover:bg-slate-700 text-emerald-400 rounded-xl border border-slate-700 text-xs font-semibold flex items-center space-x-1 ml-auto transition-all"
                    >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md animate-in fade-in">
          <div className="w-full max-w-md glass-card rounded-3xl p-6 border border-slate-700 shadow-2xl space-y-5">
            ...
            <div className="flex justify-between items-center pb-3 border-b border-slate-800">
              <h3 className="text-sm font-bold text-white">บันทึก LINE User ID ({selectedUser.fullname})</h3>
            ...
            <form onSubmit={handleSaveLineId} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full glass-input rounded-xl p-3 text-xs bg-slate-900 border-slate-700 text-white font-mono focus:outline-none"
                />
              ...
              <div className="flex justify-end space-x-2 pt-3 border-t border-slate-800">
                <button
                  ...
                  className="py-2 px-4 rounded-xl text-xs font-semibold text-slate-400 hover:bg-slate-800"
                >
                  ยกเลิก
                </button>
                <button
                  type="submit"
                  className="py-2 px-5 bg-emerald-500 hover:bg-emerald-400 text-white text-xs font-semibold rounded-xl shadow-lg shadow-emerald-500/25"
                >
                  บันทึกข้อมูล
                </button>
              </div>
