...
export default function LeaveCalendar({ 
  ...
  const getStatusBadge = (status) => {
    if (status === 'Approved') return <span className="px-2 py-0.5 rounded-full bg-emerald-100 text-emerald-700 text-[10px] md:text-xs font-bold flex items-center gap-1"><CheckCircle2 className="w-3 h-3 md:w-3.5 md:h-3.5"/> อนุมัติแล้ว</span>;
    if (status === 'Rejected') return <span className="px-2 py-0.5 rounded-full bg-rose-100 text-rose-700 text-[10px] md:text-xs font-bold flex items-center gap-1"><XCircle className="w-3 h-3 md:w-3.5 md:h-3.5"/> ไม่อนุมัติ</span>;
    return <span className="px-2 py-0.5 rounded-full bg-amber-100 text-amber-700 text-[10px] md:text-xs font-bold flex items-center gap-1"><Clock className="w-3 h-3 md:w-3.5 md:h-3.5"/> รออนุมัติ</span>;
  };
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="bg-[var(--card-bg)] p-4 md:p-6 rounded-2xl shadow-lg border border-[var(--card-border)]">
        <div className="flex justify-between items-center mb-6 gap-1 sm:gap-2">
          <button onClick={prevMonth} className="px-2 py-1.5 sm:px-3 sm:py-2 md:px-4 md:py-2 bg-blue-500 hover:bg-blue-600 text-white dark:bg-slate-800 dark:hover:bg-slate-700 dark:text-blue-400 dark:border dark:border-blue-500/30 rounded-lg flex items-center space-x-0.5 sm:space-x-2 text-[10px] sm:text-xs md:text-sm font-bold transition-colors whitespace-nowrap">
          ...
          <button onClick={nextMonth} className="px-2 py-1.5 sm:px-3 sm:py-2 md:px-4 md:py-2 bg-blue-500 hover:bg-blue-600 text-white dark:bg-slate-800 dark:hover:bg-slate-700 dark:text-blue-400 dark:border dark:border-blue-500/30 rounded-lg flex items-center space-x-0.5 sm:space-x-2 text-[10px] sm:text-xs md:text-sm font-bold transition-colors whitespace-nowrap">
          ...
          {calendarDays.map((dayNum, index) => {
            ...
            return (
              <div key={index} className={`min-h-[6rem] md:min-h-0 md:h-32 rounded-xl p-3 md:p-2 transition-all flex flex-col relative overflow-hidden ${borderClass} border`}>
                ...
                <div className="flex-1 min-h-0 flex flex-col space-y-1.5 overflow-y-auto custom-scrollbar pb-1 pr-1">
                  {Object.entries(groupedByDept).map(([dept, reqs]) => (
                    <div
                      key={dept}
                      onClick={() => openGroupModal(dateStr, dept, reqs)}
                      className="flex items-center space-x-1.5 px-2 py-1.5 rounded-lg bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-slate-700 font-bold text-[10px] md:text-xs cursor-pointer hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors truncate shadow-sm shrink-0"
                    >
                      <Users className="w-3 h-3 md:w-3.5 md:h-3.5 text-slate-500 dark:text-slate-400 flex-shrink-0" />
      ...
      {selectedGroup && (
        <div className="fixed inset-0 z-50 flex items-start md:items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm animate-in fade-in" onClick={closeGroupModal}>
          <div className="bg-white dark:bg-[var(--card-bg)] rounded-2xl shadow-2xl w-full max-w-4xl max-h-[calc(100svh-2rem)] md:max-h-[80dvh] min-h-0 flex flex-col overflow-hidden border border-slate-200 dark:border-[var(--card-border)]" onClick={e => e.stopPropagation()}>
                ...
                {selectedGroup.reqs.map(req => (
                  <div key={req.id} className="bg-white dark:bg-[var(--card-bg)] rounded-2xl p-4 border border-[var(--card-border)] shadow-sm hover:shadow-md transition-all flex flex-col gap-4">
                      ...
                      <div className="flex items-center space-x-3">
                        ...
                        <div>
                          <div className="flex items-center gap-2">
                            <span className="text-[10px] md:text-xs font-bold text-blue-500 bg-blue-50 dark:bg-blue-500/10 px-2 py-0.5 rounded-md">
                              {req.id}
                            </span>
                          </div>
                          <h3 className="font-bold text-[var(--text-main)] text-sm md:text-base mt-0.5">{req.requester ? req.requester.fullname : req.user_id}</h3>
                        </div>
                      </div>
                      
                      <div className="flex flex-col items-end gap-2">
                        <LeaveTypeBadge type={req.leave_type} size="md" />
                    ...
                    <div className="flex justify-between items-center bg-slate-50 dark:bg-slate-800/50 rounded-xl p-3 border border-slate-100 dark:border-slate-700/50">
                      ...
                      <div className="text-[10px] md:text-xs font-bold text-slate-400 dark:text-slate-500 flex flex-col items-center">
                        <span>{req.leave_duration || '1'} วัน {req.leave_period === 'Morning' ? '(เช้า)' : req.leave_period === 'Afternoon' ? '(บ่าย)' : ''}</span>
                    ...
                    <div className="flex justify-between items-center gap-2">
                      <div className="text-xs md:text-sm text-[var(--text-muted)] truncate flex-1" title={req.description}>
