      ...
      <div className="w-full max-w-2xl glass-card-clean-clean rounded-3xl p-6 md:p-8 border border-slate-200 dark:border-[var(--card-border)] shadow-2xl relative max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between pb-4 border-b border-slate-200 dark:border-[var(--card-border)] mb-6">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h3 className="text-lg font-extrabold text-[var(--text-main)] dark:text-[var(--text-main)]">ยื่นคำขอลางานใหม่</h3>
              <p className="text-xs text-[var(--text-muted)] dark:text-[var(--text-muted)]">กรอกรายละเอียดเพื่อส่งอนุมัติไปยังหัวหน้างาน</p>
            </div>
          </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="p-4 rounded-2xl bg-slate-100/80 dark:bg-[var(--card-bg)]/60 border border-slate-200 dark:border-[var(--card-border)] space-y-3">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              <div>
                <label className="block text-[11px] text-[var(--text-muted)] mb-1">ผู้อนุมัติคนที่ 1 (จำเป็น)</label>
                <select
                  required
                  value={selectedApprover1}
                  onChange={(e) => setSelectedApprover1(e.target.value)}
                  className="w-full glass-input-clean text-xs rounded-xl p-2.5"
                >
              ...
              <div>
                <label className="block text-[11px] text-[var(--text-muted)] mb-1">ผู้อนุมัติคนที่ 2 (ถ้ามี)</label>
                <select
              ...
              <div>
                <label className="block text-[11px] text-[var(--text-muted)] mb-1">ผู้อนุมัติคนที่ 3 (ถ้ามี)</label>
                <select
          ...
          <div>
            ...
            <div className="flex items-center space-x-3">
              ...
              {file && <span className="text-xs text-emerald-600 dark:text-emerald-400 font-semibold truncate max-w-xs">{file.name}</span>}
            </div>
