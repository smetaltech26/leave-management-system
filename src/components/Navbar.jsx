...
export default function Navbar({ currentUser, setCurrentUser, users, activeTab, setActiveTab, theme, toggleTheme }) {
  ...
  return (
    <nav className="glass-panel-clean sticky top-0 z-40 px-4 py-3 border-b border-slate-200 dark:border-slate-800 transition-colors">
      <div className="max-w-7xl mx-auto flex items-center justify-between">
        ...
        <div className="flex items-center space-x-3 cursor-pointer" onClick={() => setActiveTab('home')}>
          ...
          <div>
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400 hidden sm:block">ระบบจัดการคำขอลางานออนไลน์ (S Metal Tech)</p>
          </div>
        </div>
        ...
        <div className="flex items-center space-x-3">
          ...
          <button
            onClick={toggleTheme}
            className="p-2.5 rounded-2xl bg-slate-100 dark:bg-slate-800/80 text-slate-600 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700 transition-all border border-slate-200 dark:border-slate-700 flex items-center space-x-1.5 text-xs font-semibold shadow-sm"
          ...
          <div className="relative hidden md:block">
            <select
              ...
              className="glass-input-clean text-xs rounded-xl px-3 py-2 text-slate-700 dark:text-slate-200 focus:outline-none cursor-pointer border-slate-200 dark:border-slate-700"
            >
          ...
          <div className="relative">
            <button
              ...
              className="flex items-center space-x-2.5 p-1.5 rounded-2xl hover:bg-slate-100 dark:hover:bg-slate-800/60 transition-all border border-transparent hover:border-slate-200 dark:hover:border-slate-700"
            >
              <img
              ...
              <div className="text-left hidden sm:block">
                <div className="text-xs font-semibold text-slate-800 dark:text-slate-200">{currentUser?.fullname || 'ผู้ใช้งาน'}</div>
            ...
            {/* Dropdown Menu */}
            {showProfileMenu && (
              <div className="absolute right-0 mt-2 w-64 glass-card-clean rounded-2xl py-2 shadow-2xl z-50 border border-slate-200 dark:border-slate-800">
                ...
                <div className="p-1">
                  <button
                    ...
                    className="w-full text-left px-3 py-2 text-xs text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-xl flex items-center space-x-2 font-medium"
                  >
