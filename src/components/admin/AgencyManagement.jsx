...
import { Building2, Plus, Edit2, Trash2, X, Save, Search } from 'lucide-react';
...
export default function AgencyManagement({ agencies = [], setAgencies }) {
  ...
  const handleDelete = (id) => {
    if (window.confirm('คุณต้องการลบหน่วยงานนี้ออกจากระบบหรือไม่?')) {
            ...
            {filteredData.length === 0 ? (
              ...
              filteredData.map((agency, index) => (
                <tr key={agency.id || index} className="hover:bg-slate-50 dark:hover:bg-slate-800/20 transition-colors">
                  <td className="p-4 font-mono text-slate-600 dark:text-slate-400">{agency.id}</td>
