      ...
      <aside className="w-64 glass-panel border-r border-slate-800 p-4 hidden md:flex flex-col justify-between min-h-[calc(100vh-65px)]">
        <div className="space-y-6">
          ...
          <button
            ...
            className="w-full py-3 px-4 bg-gradient-to-r from-emerald-500 to-teal-600 hover:from-emerald-400 hover:to-teal-500 text-white font-semibold rounded-2xl shadow-lg shadow-emerald-500/25 flex items-center justify-center space-x-2 transition-all transform hover:scale-[1.02] active:scale-[0.98]"
          >
          ...
          <div className="space-y-1">
            <p className="px-3 text-[11px] font-semibold text-slate-500 uppercase tracking-wider mb-2">เมนูหลัก</p>
        ...
        <div className="p-3 rounded-xl bg-slate-900/60 border border-slate-800/80 text-center">
          <p className="text-[11px] text-slate-400">SMT Leave System v2.0</p>
          <p className="text-[10px] text-slate-500 mt-0.5">LINE 1:1 Messaging Integrated</p>
        </div>
      </aside>

      {/* Mobile Bottom Navigation */}
      <div className="md:hidden fixed bottom-0 left-0 right-0 glass-panel border-t border-slate-800 z-40 px-2 py-1.5 flex items-center justify-around shadow-2xl">
        {navItems.filter(item => item.show !== false).slice(0, 5).map((item) => {
          ...
          return (
            <button
              key={item.id}
              onClick={() => setActiveTab(item.id)}
              className={`flex flex-col items-center py-1 px-3 rounded-xl relative ${
