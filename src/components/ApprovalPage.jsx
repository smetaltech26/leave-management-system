    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-1">รายการคำขอลางานที่รอให้คุณตรวจสอบและอนุมัติตามลำดับขั้นตอน</p>
        </div>
      ...
      {displayList.length > 0 ? (
        <div className="grid grid-cols-1 gap-4">
          {displayList.map((req) => {
            ...
            return (
              <div key={req.id} className="glass-card rounded-2xl p-5 border border-slate-800 hover:border-slate-700 transition-all space-y-4">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-slate-800/80">
                  <div className="flex items-center space-x-3">
                    ...
                    />
                    <div>
                      <div className="font-semibold text-sm text-white">{requester?.fullname || req.user_id}</div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
                  <div>
                    ...
                    <p className="font-medium text-slate-200 mt-0.5">{req.description}</p>
                  </div>
                  <div>
                    <span className="text-slate-500">ช่วงเวลาลางาน:</span>
                    <p className="font-medium text-slate-200 mt-0.5">{req.date_start} ถึง {req.date_end}</p>
                  </div>
                </div>

                {/* Approvers Step Timeline */}
                <div className="pt-3 border-t border-slate-800/80">
                  <span className="text-[11px] font-semibold text-slate-400 block mb-2">ขั้นตอนการอนุมัติ (Approval Chain):</span>
                ...
                {activeSubTab === 'pending' && (
                  <div className="pt-3 border-t border-slate-800 flex justify-end space-x-3">
                    <button
                      ...
                      className="py-2 px-5 bg-gradient-to-r from-emerald-500 to-teal-600 hover:from-emerald-400 hover:to-teal-500 text-white text-xs font-semibold rounded-xl shadow-lg shadow-emerald-500/25 flex items-center space-x-2 transition-all"
                    >
        ...
        <div className="glass-card rounded-2xl p-12 text-center text-slate-400 space-y-2">
          ...
          <p className="text-sm font-semibold text-slate-300">ไม่มีรายการคำขอลางานที่ต้องดำเนินการในขณะนี้</p>
      ...
      {selectedRequest && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md animate-in fade-in">
          <div className="w-full max-w-lg glass-card rounded-3xl p-6 border border-slate-700 shadow-2xl space-y-5">
            
            <div className="flex justify-between items-center pb-3 border-b border-slate-800">
              <h3 className="text-base font-bold text-white">พิจารณาอนุมัติคำขอลางาน ({selectedRequest.id})</h3>
              <button onClick={() => setSelectedRequest(null)} className="text-slate-400 hover:text-white">✕</button>
            </div>

            <div className="space-y-3 text-xs">
              <div className="p-3 rounded-xl bg-slate-900 border border-slate-800">
                <p className="text-slate-400">ผู้ขอลา: <span className="text-white font-semibold">{selectedRequest.user_id}</span></p>
                <p className="text-slate-400">ประเภท: <span className="text-emerald-400 font-semibold">{selectedRequest.leave_type} ({selectedRequest.leave_duration} วัน)</span></p>
                <p className="text-slate-400">เหตุผล: <span className="text-slate-200">{selectedRequest.description}</span></p>
              </div>

              <div>
                <label className="block text-xs font-semibold text-slate-300 mb-1.5">หมายเหตุ / ความเห็นผู้อนุมัติ (Optional)</label>
                <textarea
                  rows={2}
            ...
            <div className="flex items-center justify-end space-x-3 pt-3 border-t border-slate-800">
              ...
              <button
                ...
                className="py-2.5 px-5 bg-gradient-to-r from-emerald-500 to-teal-600 hover:from-emerald-400 hover:to-teal-500 text-white text-xs font-semibold rounded-xl shadow-lg shadow-emerald-500/25 flex items-center space-x-1.5 transition-all"
              >
                <CheckCircle2 className="w-4 h-4" />
