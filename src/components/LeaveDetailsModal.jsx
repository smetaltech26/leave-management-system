...
export default function LeaveDetailsModal({ isOpen, onClose, request, user, allPolicies = [], users, agencies, departments }) {
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 sm:p-6 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-200" onClick={onClose}>
      <div className="bg-[var(--bg-main)] rounded-3xl shadow-2xl w-[90vw] md:w-full md:max-w-2xl max-h-[90vh] flex flex-col overflow-hidden border border-[var(--card-border)] relative" onClick={e => e.stopPropagation()}>
