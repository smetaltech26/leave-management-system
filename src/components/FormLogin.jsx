    ...
    <div className="min-h-screen flex items-center justify-center p-4 relative overflow-hidden bg-slate-950">
      ...
      <div className="w-full max-w-md glass-card-clean rounded-3xl p-8 border border-[var(--card-border)] shadow-2xl relative z-10">
        ...
        <div className="text-center space-y-3 mb-8">
          <div className="w-16 h-16 rounded-2xl bg-gradient-to-tr from-blue-500 to-sky-400 p-0.5 mx-auto shadow-xl shadow-blue-500/20">
            <div className="w-full h-full bg-slate-950 rounded-[14px] flex items-center justify-center">
              <span className="text-blue-400 font-extrabold text-2xl tracking-tight">SMT</span>
            </div>
          </div>
          <h2 className="text-2xl font-bold bg-gradient-to-r from-white via-slate-200 to-blue-400 bg-clip-text text-transparent">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <button
            type="submit"
            className="w-full py-3 px-4 bg-gradient-to-r from-blue-500 to-sky-600 hover:from-blue-400 hover:to-sky-500 text-[var(--text-main)] font-semibold rounded-xl shadow-lg shadow-blue-500/25 flex items-center justify-center space-x-2 transition-all transform active:scale-[0.98] mt-2"
          >
          ...
          <div className="space-y-2">
            {users.map(u => (
              <button
                key={u.id}
                onClick={() => onLogin(u)}
                className="w-full p-2.5 rounded-xl bg-[var(--card-bg)]/50 hover:bg-[var(--card-bg)]/80 border border-[var(--card-border)]/80 hover:border-blue-500/40 text-left flex items-center justify-between transition-all group"
              >
                <div className="flex items-center space-x-3">
                  <img src={u.avatar_url} alt={u.fullname} className="w-8 h-8 rounded-lg object-cover" />
                  <div>
                    <div className="text-xs font-semibold text-[var(--text-main)] group-hover:text-blue-400">{u.fullname}</div>
                    <div className="text-[10px] text-[var(--text-muted)]">{u.email}</div>
                  </div>
                </div>
                <span className="px-2 py-0.5 text-[10px] bg-[var(--card-bg)] text-[var(--text-muted)] rounded-md border border-[var(--card-border)] group-hover:bg-blue-500/20 group-hover:text-blue-400">
                  {u.role}
                </span>
              </button>
            ))}
